...
bool ChessBoard::WhiteCanMove()
{
   ...
   for ( ybase = OFFSET(2,2); ybase <= OFFSET(2,9); ybase += NORTH )
   {
      for ( x=0; x < 8; x++ )
      {
         ...
         {
            case WPAWN:
                 if ( WP_CanMove(ofs,ybase) )  {PROFILER_EXIT(); return true;}
                 break;

            case WKNIGHT:
                 if ( WN_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case WBISHOP:
                 if ( WB_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case WROOK:
                 if ( WR_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case WQUEEN:
                 if ( WQ_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;
            ...
            case WKING:
                 if ( !(flags & SF_WCHECK) )
                 {
                    ...
                    if ( WK_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 }
                 break;
         }
      }
...
bool ChessBoard::BlackCanMove()
{
   ...
   for ( ybase = OFFSET(2,9); ybase >= OFFSET(2,2); ybase += SOUTH )
   {
      for ( x=0; x < 8; x++ )
      {
         ...
         {
            case BPAWN:
                 if ( BP_CanMove(ofs,ybase) )  {PROFILER_EXIT(); return true;}
                 break;

            case BKNIGHT:
                 if ( BN_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case BBISHOP:
                 if ( BB_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case BROOK:
                 if ( BR_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;

            case BQUEEN:
                 if ( BQ_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 break;
            ...
            case BKING:
                 if ( !(flags & SF_BCHECK) )
                 {
                    ...
                    if ( BK_CanMove(ofs) )  {PROFILER_EXIT(); return true;}
                 }
                 break;
         }
      }
